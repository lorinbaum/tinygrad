...
class Linearizer(Kernel):
  ...
  render_ops = { Variable: lambda self, ops, ctx: ctx[self.expr], NumNode: lambda self, ops, ctx: UOp.const(dtypes.int, self.b),
                ...
                LtNode: lambda self, ops, ctx: uop_alu_idx(self.a.render(ops, ctx), self.b, ops, ctx, BinaryOps.CMPLT),
    SumNode: lambda self,ops,ctx:
      functools.reduce(lambda a,b: uop_alu_idx(a, b, ops, ctx, BinaryOps.ADD), self.nodes[1:], self.nodes[0].render(ops,ctx)),
    AndNode: lambda self,ops,ctx:
      functools.reduce(lambda a,b: uop_alu_idx(a, b, ops, ctx, BinaryOps.MUL), self.nodes[1:], self.nodes[0].render(ops,ctx)) }
      ...
      if key not in self.load_cache:
        if acc is not None:
          ...
          if valid.min == 0 and valid.max == 1:
            valid_rendered = valid.render(self.render_ops, self.loop_uops)
          ...
          rendered_idx = UOp(UOps.CAST, dtypes.int.vec(2), tuple(x.render(self.render_ops, self.loop_uops) for x in image_idx))
          valid_tuple = (valid.render(self.render_ops, self.loop_uops), UOp.const(buf.dtype.base.vec(4), invalid_value)) if valid.min == 0 else ()
          ...
          if localtype == localtype.scalar():
            idx_small = idx%4
            res = idx_small.render(self.render_ops, self.loop_uops)
          ...
          buf_uop = self.buf_uops[i]
          assert buf_uop is not None, f"buffer {i} wasn't UOped"
          rendered_idx = idx.render(self.render_ops, self.loop_uops)
          valid_tuple = (valid.render(self.render_ops, self.loop_uops), UOp.const(localtype, invalid_value)) if valid.min == 0 else tuple()
    ...
    for _idx, var in store_offset.items():
      ...
      if isinstance(buf.dtype, ImageDType):
        ...
        rendered_idx = UOp(UOps.CAST, dtypes.int.vec(2), \
                      tuple(x.render(self.render_ops, self.loop_uops) for x in image_idx))
      else:
        rendered_idx = idx.render(self.render_ops, self.loop_uops)
      if valid.min == 1: stores.append(UOp(UOps.STORE, None, (buf_uop, rendered_idx, var)))
      else: stores.append(UOp(UOps.STORE, None, (buf_uop, rendered_idx, var, valid.render(self.render_ops, self.loop_uops))))
    return stores

  # render loop
  def render_loop(self, xx:List[Variable], depth:int) -> Tuple[UOp, ...]:
    new_loops = {x.expr:UOp(UOps.RANGE, dtypes.int32, (
      UOp.const(dtypes.int, x.min) if isinstance(x.min, int) else cast(Node, x.min).render(self.render_ops, self.loop_uops),
      UOp.const(dtypes.int, x.max+1) if isinstance(x.max, int) else cast(Node, x.max+1).render(self.render_ops, self.loop_uops)), arg=(depth,i)) for i,x in enumerate(xx) if not isinstance(x, NumNode) and x.expr is not None}  # noqa: E501
  ...
  def render_reduceop(self, reduceop:LazyOp, accs:Dict[LazyOp, List[UOp]], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]],
    ...
    if (tc:=self.tensor_core):
      ...
      def gate_acc(r, idxs): return [
        UOp.alu(TernaryOps.WHERE, valid.render(self.render_ops, self.loop_uops), acc, UOp.const(r.dtype, 0)) if valid.min == 0 and valid.max == 1
        else acc for valid, acc in zip(expand_node(self.sts[self.full_buf_index].expr_idxs(idxs)[1], expand_idxs(idxs)), accs[r])]
    ...
    if self.group_for_reduces:
      ...
      if self.opts.has_local:
        ...
        if_cond: UOp = create_lt_node(self.sts[-1].expr_idxs(fake_idxs)[0], 1).render(self.render_ops, self.loop_uops)
